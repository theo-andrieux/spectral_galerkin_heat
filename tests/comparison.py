...
def load_temperature_profiles(
    out_dir=".validation", validation_dir=".validation", file_tag="spectral"
):
    ...
    for direction in ["x", "y", "z"]:
        ...
        if os.path.exists(filepath):
            ...
            print(
                f"Loaded {filepath}: {np.sum(mask)} valid points (filtered {np.sum(~mask)} NaN)"
            )
        else:
            print(f"Warning: {filepath} not found")
...
def plot_comparison(
    profiles, output_file="temperature_comparison.png", label_sim="Simulation"
):
    ...
    for direction, label in zip(directions, labels):
        ...
        if len(coords_FE) > 1:
            ...
            T_FE_interp = np.interp(
                coords_spec, coords_FE_sorted, T_FE_sorted, left=np.nan, right=np.nan
            )
            ...
            error_T[safe_mask] = (
                np.abs(T_spec[safe_mask] - T_FE_interp[safe_mask])
                / denom[safe_mask]
                * 100.0
            )
        ...
        if len(coords_spec) > 1 and len(coords_FE) > 1:
            ...
            dT_FE_interp = np.interp(
                coords_spec, coords_FE_sorted, dT_FE, left=np.nan, right=np.nan
            )
            ...
            error_dT[safe_mask_dT] = (
                np.abs(dT_spec[safe_mask_dT] - dT_FE_interp[safe_mask_dT])
                / denom_dT[safe_mask_dT]
                * 100.0
            )
...
def apply_transforms(
    profiles, align_target="none", invert_target="none", invert_axes=()
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare spectral and FE temperature profiles."
    )
    parser.add_argument(
        "--out-dir", default=".validation", help="Directory with spectral outputs"
    )
    parser.add_argument(
        "--validation-dir", default=".validation", help="Directory with FE outputs"
    )
    parser.add_argument(
        "--output-figure",
        default="temperature_comparison.png",
        help="Output figure file",
    )
    parser.add_argument(
        "--align-start",
        action="store_true",
        help="Translate one or both profiles so their starts align",
    )
    parser.add_argument(
        "--align-target",
        choices=["none", "FE", "spectral", "both"],
        default="FE",
    ...
    profiles = load_temperature_profiles(
        out_dir=args.out_dir, validation_dir=args.validation_dir, file_tag=args.file_tag
    )

    # Parse invert axes
    invert_axes = tuple(
        [s.strip().lower() for s in args.invert_axes.split(",") if s.strip()]
    )
    ...
    profiles = apply_transforms(
        profiles,
        align_target=align_target,
        invert_target=args.invert_target,
