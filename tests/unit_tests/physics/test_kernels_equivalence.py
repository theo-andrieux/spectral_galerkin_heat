...
def test_dct_idct_parity(gpu):
    ...
    np.testing.assert_allclose(
        cupy.asnumpy(gpu_mod.DCT_II(cupy.asarray(q))),
        cpu.DCT_II(q),
        rtol=RTOL,
        atol=1e-4,
    )
    ...
    np.testing.assert_allclose(
        cupy.asnumpy(gpu_mod.IDCT_II(cupy.asarray(a))),
        cpu.IDCT_II(a),
        rtol=RTOL,
        atol=1e-4,
    )
...
def test_gaussian_laser_flux_parity(gpu):
    ...
    out_gpu = gpu_mod.compute_gaussian_laser_flux(
        cupy.asarray(x), cupy.asarray(y), *args
    )
...
def test_evaporation_flux_parity(gpu):
    ...
    consts = dict(
        P0=101325.0, T_boil=3090.0, DeltaH_LV=7.41e6, R_v=150.774, T_liquidus=1800.0
    )
...
def test_update_modes_etd1_parity(gpu):
    ...
    gpu_mod.update_modes_etd1(
        cupy.asarray(aK), cupy.asarray(KK), cupy.asarray(Cp), cupy.asarray(B), out_gpu
    )
    np.testing.assert_allclose(cupy.asnumpy(out_gpu), out_cpu, rtol=RTOL)
