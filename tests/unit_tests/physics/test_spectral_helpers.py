...
def test_calculate_subgrid_indices(pos, expected):
    assert (
        hp._calculate_subgrid_indices(pos, dx=1.0, n_total_fine=20, n_box=4) == expected
    )


def test_calculate_subgrid_indices_box_bigger_than_grid():
    start, end, _ = hp._calculate_subgrid_indices(
        5.0, dx=1.0, n_total_fine=20, n_box=30
    )
...
def test_cosine_basis_shape_and_values():
    ...
    np.testing.assert_allclose(
        B, np.cos(np.pi * np.arange(3)[:, None] * coords[None, :] / 2.0)
    )
...
def test_at_points_rejects_bad_coords_shape():
    ...
    with pytest.raises(ValueError):
        hp.reconstruct_temperature_volume_at_points(
            None, None, None, None, np.zeros((5, 2))
        )
...
def test_dct_reconstruction_matches_legacy_tensor_product(
    tiny_context, state_with_modes
):
...
def test_save_temp_profiles_laser_center_requires_position():
    ...
    with pytest.raises(ValueError):
        hp.save_temp_profiles(
            None, None, _geom(), None, laser_position=None, center="laser"
        )
...
def test_save_temp_profiles_unknown_center_raises():
    ...
    with pytest.raises(ValueError):
        hp.save_temp_profiles(
            None, None, _geom(), None, laser_position=(0.0, 0.0), center="bogus"
        )
