...
def _build_and_solve_fe():
    ...
    top = [
        t
        for d, t in bnds
        if math.isclose(occ.get_center_of_mass(d, t)[2], _LZ, abs_tol=tol)
    ]
    bot = [
        t
        for d, t in bnds
        if math.isclose(occ.get_center_of_mass(d, t)[2], 0.0, abs_tol=tol)
    ]
...
def _run_sg():
    ...
    class _CVLaser(LaserPath):
        def get_state(self, t, dt):
            return LaserState(
                x=_X_START + _V * t, y=_Y0, power=_P, is_on=True, v=(_V, 0.0)
            )
...
def test_matches_fenics_nonlinear(tmp_path):
    ...
    write_structured_fields(
        sg_base, {"temperature": T_sg}, x, y, z, time=_N_STEPS * _DT
    )
